    ...
    useEffect(() => {
        async function loadCommunes() {
            try {
                ...
                // Use direct fetch to remove any possible API helper issue.
                ...
                if (Array.isArray(data)) {
                    items = data.map(normalizeCommune)
                }

                // Fallback to GeoJSON if needed.
                if (!items.length) {
                    response = await fetch('/api/communes/geojson')
                ...
                console.log('Loaded communes for search:', items)
    ...
    const results = useMemo(() => {
        ...
        const matched = communes
            .map((commune) => {
                ...
                // Search the whole object too, so even if field names change, search still works.
                    ...
                    {!loading && !loadError && results.length > 0 && (
                        results.map((commune, index) => (
                            <button
                                ...
                                className={`w-full text-left px-3 py-2 text-sm ${index === activeIndex
